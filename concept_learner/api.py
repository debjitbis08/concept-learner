...
class ConceptAPI:
    ...
    def complete_analogy(self, A: Any, B: Any, C: Any, top_k: int = 5) -> AnalogyResult:
        ...
        with torch.no_grad():
            ...
            A_z = self.model.encode(A_desc, A_mask)["z_q"]  # (1,D)
            B_z = self.model.encode(B_desc, B_mask)["z_q"]  # (1,D)
            C_z = self.model.encode(C_desc, C_mask)["z_q"]  # (1,D)
            D_z = self.model.encode(D_desc, D_mask)["z_q"]  # (N,D)
            ...
            topk_score, topk_idx = torch.topk(sim, k=min(top_k, n))
        ...
        return AnalogyResult(prediction=pred, scores=scores, concepts={})
